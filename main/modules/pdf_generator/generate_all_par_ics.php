<?php
    ...
    if(isset($_GET["h"]) && isset($_GET["m"]) && isset($_GET["y"]) && isset($_GET["type"])){
        if($_GET["h"] != md5($_GET["m"].$_GET["y"].$_GET["type"])){
    ...
    $r = DB::run("SELECT * FROM request r JOIN user_accounts ua ON r.uid = ua.uid WHERE r.created_at LIKE ? AND r.ics_par LIKE ?", ["%" . $qDate . "%", "%" . $_GET["type"] . "%"]);
    while($row = $r->fetch()){
    ...
    // This method has several options, check the source code documentation for more information.
    if($_GET["type"] == "par"){
        $filename = "all_par_" . $_GET["y"] . "-" . $_GET["m"] . ".pdf"; 
    }else{
        $filename = "all_ics_" . $_GET["y"] . "-" . $_GET["m"] . ".pdf"; 
    }
