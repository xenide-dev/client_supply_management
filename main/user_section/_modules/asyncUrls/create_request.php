    ...
    if(isset($_POST["type"])){
        if($_POST["type"] == "item"){
            if($_POST["operation"] == "getAll"){
                ...
                if(isset($_SESSION["create_request_type"])){
                    if($_SESSION["create_request_type"] == "request"){
                        // retrieve all the items under user's ppmp
                ...
                $output["info"] = $arow;
                $output["msg"] = true;
            }elseif($_POST["operation"] == "getItemPPMP"){
                $id = $_POST["id"];
                // retrieve item
                $a = DB::run("SELECT * FROM item_dictionary id LEFT JOIN item_category ic ON id.catid = ic.catid WHERE id.itemid = ? ORDER BY id.item_name ASC", [$id]);
                $arow = $a->fetch();
